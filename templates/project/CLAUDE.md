...
| `consent/` | Voice/visual clone consent records. Required before the avatar step will clone anyone. |
...
person's voice or face without a matching, filled-in file in `consent/`. A fresh,
synthetic avatar needs no such record.
...
`--dry-run` (show the plan, make no network call). All honor `NO_COLOR`.
