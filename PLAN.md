...
- `features.feedback` is off by default and independent of `followUpEmail`. Turning it on makes the agent draw feedback out in conversation and email a `SESSIONFEEDBACK` insight after the session ends, which is also a new purpose under section 10 and needs its own privacy-panel copy.
...
- **Optional post-provision stages: follow-up email and feedback capture.** When `features.followUpEmail` is on, the engine creates two session-lifecycle rules and a branded email template extracting contact details. When `features.feedback` is on, it does the same for a `SESSIONFEEDBACK` insight, with its own template and lifecycle rules so the two features never collide. Both run after step 9, are skipped by default, and are the only parts of provisioning that touch a second Kaltura API. The appendix carries the contract.
...
The update commands are the same nine steps addressed one at a time, and there are seven of them rather than one per field: `update-prompts` (directive, glossary, prompt blocks, persona name), `update-capabilities`, `update-avatar` (opening phrase, voice, visual, motion), `update-agent` (display name, tags, session length), `attach-tool` (one generic command, any client tool), `update-followup` (lifecycle rules and email template), and `update-feedback` (lifecycle rules and email template, for the separate feedback-capture feature). All seven share the read-compare-write-verify shape in the appendix, all seven take `--dry-run`, and all seven exit non-zero when the post-write read-back disagrees.
...
- **The feedback feature is a separate, also off-by-default toggle.** Turned on, the agent draws feedback out in conversation (what stood out, what could be better, whether they'd recommend it), and the platform extracts it as a `SESSIONFEEDBACK` insight from the full transcript after the session ends, emailed to configured recipients. It uses its own insight key, never `FEEDBACK` (the key `followUpEmail`'s rule already writes), so both features can be on at once without two lifecycle rules racing to write the same key. Independent of `followUpEmail`: a project can capture feedback without collecting contact details. `features.feedback` must be set explicitly, and it needs its own privacy-panel paragraph before section 6.7 will bundle, same as `followUpEmail`.
...
1. **What the tool does by default:** disclosure line, synthetic-content label when cloning, no audience capture, session-only transcripts, captions on, no contact collection and no follow-up email, no feedback capture.
