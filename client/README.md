...
This costs real API calls against a real account. `.github/workflows/live-e2e.yml` runs it only on a manual `workflow_dispatch` by someone with write access, never on push or pull request. It uses the `KALTURA_PARTNER_ID`/`KALTURA_ADMIN_SECRET` repo secrets to rebuild `dist.html`, and the `DEMO_PROVISIONING_STATE` secret for the demo's live ids, so it tests the same agent the maintainers update. After any command that changes `demo/.provisioning-state.json`, refresh that secret with `gh secret set DEMO_PROVISIONING_STATE < demo/.provisioning-state.json`. On failure, each test prints its masked debug timeline (`window.__debugTimeline`) to the log. Push and pull request runs get the secretless checks in `ci.yml`. Locally, put the same two values in a root `.env` (see `.env.example`).
...
The avatar server sends only H264 video. Playwright's Linux Firefox has no H264 until it downloads the OpenH264 plugin, so the Firefox project turns that download on and each worker waits up to 120s for H264 before its first test (`test/e2e/helpers.mjs`). Expect the first Firefox test to start 20 to 40 seconds late.

`@playwright/test` is pinned to 1.62.1. In 1.63.0, Firefox never starts WebRTC ICE, so every Firefox session fails. Run the Live E2E workflow before you upgrade it.
