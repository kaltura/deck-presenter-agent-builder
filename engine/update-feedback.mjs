#!/usr/bin/env node
/**
 * Provisions the optional conversational feedback capture: a reusable
 * InsightSettings entity, a branded email template (mgmt.emailTemplates),
 ...
 * rules both writing the same insight key on the same session_ended event is
 * an undocumented, unverifiable race. A distinct key makes that structurally
 * impossible instead of just unlikely, so both features can be on at once.
 *
 ...
 * an edge case.
 *
 * Usage: node engine/update-feedback.mjs --project <path> [--dry-run] [--yes] [--json]
 */
...
const REQUIRED_KEYS = ['SUMMARY', 'SESSIONFEEDBACK'];
...
async function main() {
  ...
  if (!project.features?.feedback) {
    progress(flags, 'features.feedback is off in project.json. Nothing to do.');
    result(flags, { skipped: true, reason: 'feedback feature is off' });
    return;
  }

  const feedback = project.feedback || {};
  if (!Array.isArray(feedback.recipients) || feedback.recipients.length === 0) {
    fail(flags, EXIT.VALIDATION, 'features.feedback is on but project.json.feedback.recipients is empty. Set at least one Kaltura user id (often the account login email).');
  }
  const { mgmt, partnerId } = connect(projectRoot, flags);
  const creds = loadCredentials(projectRoot);
  if (!creds.messagingUrl) {
    fail(flags, EXIT.CREDENTIAL, 'features.feedback is on but KALTURA_MESSAGING_URL is not set in .env. Ask your Kaltura account contact for this account\'s Messaging API base URL.');
  }
  ...
  const insightSettingsPlan = await planInsightSettings(mgmt, ks, content.FEEDBACK_INSIGHT_SETTINGS);

  const ruleASystemName = content.feedbackLifecycleRuleA([]).systemName;
  const ruleA = await findRuleBySystemName(mgmt, ks, ruleASystemName);
  // ruleA's desired action depends on the InsightSettings id resolved
  ...
  const ruleBSystemName = content.feedbackLifecycleRuleB('placeholder').systemName;
  ...
  const planLines = [`Sync feedback capture for project "${state.slug}" (agentId ${agentId}):`];
  ...
  if (templateAction === 'unchanged') {
    ...
    const updated = await mgmt.emailTemplates.update(existingTemplate.id, content.FEEDBACK_EMAIL_TEMPLATE, ks);
    ...
    const created = await mgmt.emailTemplates.create({ appGuid, ...content.FEEDBACK_EMAIL_TEMPLATE }, ks);
    templateId = created.id;
  }
  recordStep(projectRoot, state, 'feedbackTemplateId', { value: templateId, origin: 'created' });

  const insightSettingIds = await executeInsightSettings(mgmt, ks, flags, insightSettingsPlan);
  recordStep(projectRoot, state, 'feedbackInsightSettingIds', { value: insightSettingIds, origin: 'created' });

  const desiredRuleA = content.feedbackLifecycleRuleA(Object.values(insightSettingIds));
  ...
  recordStep(projectRoot, state, 'feedbackLifecycleRuleAId', { value: ruleAId, origin: 'created' });

  const desiredRuleB = content.feedbackLifecycleRuleB(templateId);
  ...
  recordStep(projectRoot, state, 'feedbackLifecycleRuleBId', { value: ruleBId, origin: 'created' });

  // ── Verify: dry-run match both rules ──
  const errors = [];
  ...
  progress(flags, 'Verified: email template, insight settings, and both lifecycle rules are wired and dry-run matching.');
