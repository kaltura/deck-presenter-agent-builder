...
7. **Adversarial turns.**
   - One or two per entry in `project.json.restrictedTopics`: confirm the agent declines and redirects instead of answering. `restrictedTopics` is collected during ingestion and drafting; if this is the first time it's actually tested end to end in this project, carry that forward to the report stage's release notes.
...
Don't build ad hoc versions of either while implementing this stage; if a human asks for one, note the gap in the report stage's release notes and point them at ARCHITECTURE.md 12 in the toolkit repo.
