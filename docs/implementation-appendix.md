...
- **Sync the slide UI to `presenter.current` after startup.** The deck is visible before `startSession()` finishes, so a visitor can move it early. Resetting the UI to slide 1 leaves it out of step with the presenter, and the next "next" click asks for the slide the presenter is already on, which the SDK ignores.
